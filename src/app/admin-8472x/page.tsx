...
export default function AdminPage() {
  ...
  const [tasks, setTasks] = useState([
    { id: 1, title: 'Follow on X (Twitter)', points: 20, completions: 124 },
    { id: 2, title: 'Join Telegram', points: 25, completions: 89 },
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-[calc(100vh-100px)] flex items-center justify-center px-4 pb-28 md:pb-0">
        <div className="w-full max-w-sm">
          <div className="bg-card border border-card-border rounded-2xl p-8">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              {error && (
                <div className="bg-red-500/10 border border-red-500/30 text-red-400 text-sm rounded-xl px-4 py-3 text-center font-medium">
                  {error}
                </div>
              )}
              ...
              <button
                type="submit"
                className="w-full bg-primary hover:bg-primary-hover text-black font-bold py-3 rounded-xl transition-colors mt-2"
              >
                Login
              </button>
            </form>
  ...
  // ========== ADMIN DASHBOARD ==========
  return (
    <div className="min-h-screen bg-background p-6 md:p-12 pb-28 md:pb-12">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-3">
            ...
            <Link href="/rewards" className="bg-card border border-card-border hover:border-primary px-4 py-2 rounded-xl text-center transition-colors text-sm font-medium">
              Back to App
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <form onSubmit={handleAddTask} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-background border border-card-border rounded-xl px-4 py-2.5 text-foreground focus:outline-none focus:border-primary transition-colors"
              ...
              <button
                type="submit"
                className="bg-primary hover:bg-primary-hover text-black font-bold py-2.5 rounded-xl mt-2 transition-colors"
              >
                Create Task
              </button>
            </form>
          ...
          <div className="bg-card border border-card-border p-6 rounded-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                <p className="text-foreground/60 text-sm">Tasks Completed</p>
                <p className="text-2xl font-bold text-green-400">8,932</p>
              </div>
              <div>
                <p className="text-foreground/60 text-sm">Points Given</p>
                <p className="text-2xl font-bold text-primary">450,200</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card border border-card-border rounded-2xl overflow-hidden">
          ...
          <div className="w-full overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white/5 border-b border-card-border text-foreground/70">
                  <th className="p-4 font-medium">Task</th>
                  <th className="p-4 font-medium">Points</th>
                  <th className="p-4 font-medium">Completions</th>
                  ...
                  <tr key={task.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 px-3 py-1 bg-red-400/10 rounded-md text-sm font-medium transition-colors"
                      >
                        Delete
                      </button>
                    </td>
